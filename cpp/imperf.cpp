 ...
 * 
 * Objective: Identification of imperfect repeats
 * Branch: backtracks to check if the upstream sequence is a valid
 *         extension
 * 
*/
...
/*
    ...
    *  @return vector of insert length and least mutations
*/
vector<uint> backtrack_repeat(utils::RepeatTracker rtracker, uint m, float fraction_mutations) {
    string valid_motif = "";
    for (int i=m-1; i>=0; i--) {
    ...
    if ((muts == rlen * fraction_mutations) && (ulen * fraction_mutations < 1)) {
        least_muts = 0, final_ulen = 0;
        return { least_muts, final_ulen };
    }

    uint threshold_muts = fraction_mutations * (rlen + ulen);
    int remain_muts  = ulen/m;

    if (debug) {

        cout << "\n*** Checking upstream sequence ***\n";
    ...
    if (debug) {
        cout << "*** Upstream inclusion:  " << final_ulen << " ***\n";
